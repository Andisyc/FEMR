...
if [[ $# -lt 2 ]]; then
  ...
  echo "FRS-TRAIN-v020 uses fixed Actor LR=3e-6 and Critic LR=1e-5; shared/adaptive overrides are rejected."
  echo "Example:"
...
if [[ ("${MODE}" == "train" || "${MODE}" == "policy_quality_eval") && -z "${FRONTRES_V015_K_CURRICULUM}" ]]; then
  echo "FRS-TRAIN-v020 requires an explicit ten-field K/M/DR schedule; no hidden DR defaults are allowed" >&2
  exit 4
fi
...
if [[ -n "${FRONTRES_V015_RESUME_CHECKPOINT}" && ! -f "${FRONTRES_V015_RESUME_CHECKPOINT}" ]]; then
  echo "checkpoint-v15 resume checkpoint not found: ${FRONTRES_V015_RESUME_CHECKPOINT}" >&2
  exit 2
fi
...
if [[ "${MODE}" == "train" && -z "${FRONTRES_V015_RESUME_CHECKPOINT}" && "${NUM_ENVS}" != "16" ]]; then
  echo "FRS-TRAIN-v020 fresh K8/M4 campaign requires NUM_ENVS=16" >&2
  exit 4
fi
...
case "${MODE}" in
  ...
  single_update|update_loop)
    echo "FRS-PPO-v008 rejects retired optimizer-writing Stage 3 mode: ${MODE}" >&2
    exit 4
    ;;
