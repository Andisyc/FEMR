...
if [[ $# -lt 2 ]]; then
  ...
  echo "FRS-TRAIN-v019 uses fixed Actor LR=3e-6 and Critic LR=1e-5; shared/adaptive overrides are rejected."
  echo "Example:"
...
if [[ -n "${FRONTRES_V015_RESUME_CHECKPOINT}" && ! -f "${FRONTRES_V015_RESUME_CHECKPOINT}" ]]; then
  echo "checkpoint-v14 resume checkpoint not found: ${FRONTRES_V015_RESUME_CHECKPOINT}" >&2
  exit 2
fi
...
if [[ "${MODE}" == "train" && -z "${FRONTRES_V015_RESUME_CHECKPOINT}" && "${NUM_ENVS}" != "16" ]]; then
  echo "FRS-TRAIN-v019 fresh K8/M4 campaign requires NUM_ENVS=16" >&2
  exit 4
fi
  ...
  for arg in "${EXTRA_TRAIN_ARGS[@]}"; do
    case "${arg}" in
      --resume|--resume=*|--resume_student_checkpoint|--resume_student_checkpoint=*|--is_full_resume|--is_full_resume=*)
        echo "v019 Stage 3 forbids legacy resume arguments: ${arg}" >&2
        exit 4
        ;;
    esac
  done
