...
FRONTRES_OUTER_REPLAY_SCHEMA = "frontres-outer-scenario-replay-v5"
...
class FrontRESScenarioReplayRecord:
    ...
    def validate(self) -> None:
        ...
        if tuple(k for k, _ in self.critic_calibration_score_by_k) != tuple(
            ...
            raise ValueError("TRAIN-v024 replay score maps must contain identical K identities")
    ...
    def score_for_k(self, horizon_k: int, *, score_kind: str = "critic_calibration") -> float | None:
        if score_kind == "critic_calibration":
            ...
            raise ValueError(f"unknown TRAIN-v024 replay score kind {score_kind!r}")
    ...
    def with_visit(
        ...
        *,
        ...
        if any(not math.isfinite(value) or value < 0.0 for value in values):
            raise ValueError("TRAIN-v024 replay values must be finite and nonnegative")
        ...
        repair_scores[int(horizon_k)] = float(repair_spread_value)
        result = replace(
            self,
...
class FrontRESOuterReplayPlan:
    ...
    def validate(self) -> None:
        ...
        if not isinstance(self.curriculum, FrontRESKStageIdentity):
            raise TypeError("outer replay plan requires one immutable TRAIN-v024 curriculum identity")
        if self.phase_name not in {"low_dr_joint_init", "coupled_ramp", "joint"}:
            raise ValueError("outer replay plan has an incompatible TRAIN-v024 phase")
...
class FrontRESOuterReplayCandidate:
    ...
    def validate(self) -> None:
        ...
        if self.current_sample_counts != (4,) * FRONTRES_OUTER_REPLAY_SCENARIO_BATCH:
            raise ValueError("candidate current sample counts must equal exact M4")
        digests = []
...
class FrontRESOuterScenarioReplay:
    ...
    def plan(
        ...
        *,
        ...
        if not isinstance(curriculum, FrontRESKStageIdentity):
            raise TypeError("outer replay plan requires a TRAIN-v024 curriculum identity")
    ...
    def stage(
        ...
        *,
        ...
        if (
            ...
            raise ValueError("FRS-TRAIN-v024 Replay requires the sealed exact M4 identity")
        ...
        if (
            ...
            raise ValueError("outer replay stage requires exact B8 x M utility/value evidence")
        if (
            ...
            raise ValueError("outer replay stage requires finite row-aligned current evidence")
        ...
        for values in grouped_old_values:
        ...
        current_sample_counts: list[int] = []
        for index, (selection, key, utility_group, value_group, spread_value) in enumerate(
            zip(
                plan.selections,
                keys,
                grouped_utilities,
            ...
            target = float(utility_group.mean().item())
            outcome_variance = float(utility_group.var(unbiased=True).item())
            standard_error = math.sqrt(outcome_variance / int(active_m))
            confidence = FRONTRES_REPLAY_CONFIDENCE_Z * standard_error
            ...
            current_utility_means.append(target)
            outcome_variances.append(outcome_variance)
            standard_errors.append(standard_error)
            confidence_half_widths.append(confidence)
            current_sample_counts.append(int(active_m))
            if existing is None:
        ...
        candidate = FrontRESOuterReplayCandidate(
            ...
            current_sample_counts=tuple(current_sample_counts),
    ...
    def commit(self, candidate: FrontRESOuterReplayCandidate, *, receipt: Mapping[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "current_sample_counts": candidate.current_sample_counts,
    ...
    def stats(self, *, active_k: int, score_kind: str = "critic_calibration") -> dict[str, int | float]:
        ...
        return {
            ...
            "minimum_active_visits": min(
                (record.visit_count for record in active_records),
                default=0,
            ),
