...
class FrontRESValueNormalizerState:
    ...
    def validate(self) -> "FrontRESValueNormalizerState":
        ...
        if not all(math.isfinite(value) for value in values):
            raise FloatingPointError("FRS-PPO-v007 value-normalizer state must be finite")
        ...
        if self.second_moment < 0.0 or self.second_moment + tolerance < self.mean**2:
            raise ValueError("FRS-PPO-v007 value-normalizer second moment is inconsistent with its mean")
        if not isinstance(self.update_count, int) or isinstance(self.update_count, bool) or self.update_count < 0:
            raise ValueError("FRS-PPO-v007 value-normalizer update_count must be a non-negative integer")
        return self
    ...
    def from_state_dict(cls, payload: Mapping[str, Any]) -> "FrontRESValueNormalizerState":
        if not isinstance(payload, Mapping) or set(payload) != {
            ...
            raise ValueError("FRS-PPO-v007 value-normalizer state has an incompatible schema")
        if payload.get("normalization_id") != FRONTRES_VALUE_NORMALIZATION_ID:
            raise ValueError("FRS-PPO-v007 value-normalizer identity mismatch")
        try:
            state = cls(
            ...
            raise ValueError("FRS-PPO-v007 value-normalizer state contains invalid values") from exc
...
def preview_frontres_v007_value_normalization(
    ...
    *,
    ...
    if not isinstance(segment_targets, torch.Tensor) or segment_targets.ndim != 1 or segment_targets.numel() != 2:
        raise ValueError("FRS-PPO-v007 value normalization requires exactly two Segment targets")
    if not bool(torch.isfinite(segment_targets).all().item()):
        raise FloatingPointError("FRS-PPO-v007 value normalization requires finite Segment targets")
    if not isinstance(state, FrontRESValueNormalizerState):
        raise TypeError("FRS-PPO-v007 value normalization requires an immutable normalizer state")
    state.validate()
    decay = float(decay)
    scale_floor = float(scale_floor)
    if not math.isfinite(decay) or not 0.0 <= decay < 1.0:
        raise ValueError("FRS-PPO-v007 value-normalizer decay must be finite in [0,1)")
    if not math.isfinite(scale_floor) or scale_floor < 1.0:
        raise ValueError("FRS-PPO-v007 value-normalizer scale floor must be finite and at least one")
    ...
    if not math.isfinite(scale) or scale < 1.0:
        raise FloatingPointError("FRS-PPO-v007 produced an invalid Critic target scale")
