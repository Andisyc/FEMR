"""Fixed TRAIN-v021 mapping from raw Recovery-Aware Gain to learning utility."""
...
def frontres_symmetric_log_utility(raw_returns: torch.Tensor) -> torch.Tensor:
    ...
    if not isinstance(raw_returns, torch.Tensor) or not raw_returns.is_floating_point():
        raise ValueError("TRAIN-v021 utility requires a floating-point tensor")
    if raw_returns.requires_grad:
        raise ValueError("TRAIN-v021 utility requires detached raw Gain")
    if not bool(torch.isfinite(raw_returns).all().item()):
        raise FloatingPointError("TRAIN-v021 utility requires finite raw Gain")
    ...
    if not bool(torch.isfinite(utility).all().item()):
        raise FloatingPointError("TRAIN-v021 utility produced non-finite values")
