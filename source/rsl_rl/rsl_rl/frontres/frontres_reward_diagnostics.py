...
def accumulate_frontres_reward_diagnostics(
    ...
    if locs.get("_r_z") is not None:
        ...
        accept_pref_mask = locs.get("_accept_pref_mask", None)
        if accept_pref_mask is not None:
            sums["accept_pref_mask"] += _mean_item((accept_pref_mask[:n_exec].sum(dim=-1) > 0).float())
        sums["accept_pref_full"] += _item(locs.get("_pref_full_frac", 0.0))
        ...
        sums["accept_pref_target"] += _item(locs.get("_pref_target_mean", 0.0))
        sums["inertial_pref_penalty_rho"] += _item(locs.get("_pref_inertial_penalty_rho_mean", 0.0))
        sums["inertial_pref_penalty_one"] += _item(locs.get("_pref_inertial_penalty_one_mean", 0.0))
