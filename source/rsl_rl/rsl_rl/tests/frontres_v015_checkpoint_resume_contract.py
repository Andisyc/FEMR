...
def _runner(layout_module, policy_cls, *, offsets=(1, 3), iteration: int = 7):
...
def test_t_checkpoint_layout_and_committed_receipt(layout_module, checkpointing, policy_cls) -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert identity["format"] == "frontres-v015-checkpoint-v1"
        assert identity["gain_contract_id"] == "FRS-GAIN-v003"
        assert identity["future_intent_layout"]["future_offsets"] == (1, 3)
        ...
        print("[T-checkpoint/T-layout/T-commit-receipt] exact q29 layout, prefix stats, and metadata-only completed receipt round-trip", flush=True)
...
def test_t_resume_rejects_layout_legacy_and_normalizer_before_mutation(layout_module, checkpointing, policy_cls) -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        mismatch = _runner(layout_module, policy_cls, offsets=(1, 2), iteration=0)
        actor_before = mismatch.alg.policy.residual_actor.weight.detach().clone()
        _expect_error(lambda: checkpointing.load_runner(mismatch, str(source_path), load_optimizer=False), "future-intent layout mismatch")
        ...
        tampered_payload["obs_norm_state_dict"]["_mean"][..., 0] += 1.0
        ...
        print("[T-resume/T-legacy-reject/T-normalizer] same-tail H mismatch, old [H,65], and tampered stats reject before mutable restore", flush=True)
