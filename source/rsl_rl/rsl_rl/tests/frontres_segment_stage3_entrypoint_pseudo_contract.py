...
def _alg_cfg() -> SimpleNamespace:
    return SimpleNamespace(
        ...
        frontres_critic_value_normalization="unset",
        frontres_critic_value_normalizer_decay=0.0,
        frontres_critic_value_normalizer_scale_floor=0.0,
...
def test_stage3_default_enters_live_train_config_without_zeroing_iterations() -> None:
    ...
    assert alg.frontres_critic_value_normalization == "ema-target-std-nonamplifying-v1"
    assert alg.frontres_critic_value_normalizer_decay == 0.9
    assert alg.frontres_critic_value_normalizer_scale_floor == 1.0
...
def test_stage3_policy_quality_config_is_formal_and_read_only() -> None:
    ...
    assert alg.frontres_critic_value_normalizer_scale_floor == 1.0
    assert alg.frontres_segment_live_update_steps == 1
    assert alg.frontres_segment_k_curriculum[1][:2] == (16, 4)
    assert alg.frontres_future_offsets == (1, 2)
...
def test_retired_optimizer_flags_reject_before_stage3_config_mutation() -> None:
    for field in (
        ...
        try:
            ...
            assert "FRS-PPO-v012 rejects retired Stage-3" in str(exc)
        else:
...
def test_stage3_ppo_schedule_rejects_adaptive() -> None:
    ...
    try:
        ...
        raise AssertionError("FRS-TRAIN-v024 must reject adaptive Stage-3 scheduling")
...
def test_stage3_split_lr_override_rejects_shared_and_partial_inputs() -> None:
    ...
    try:
        ...
        raise AssertionError("FRS-TRAIN-v024 must reject the shared LR option")
    try:
        _apply_frontres_segment_split_lr_override(
            agent_cfg,
        ...
        raise AssertionError("FRS-TRAIN-v024 must reject a partial split-LR override")
