...
def main() -> None:
    ...
    assert "FRS-PPO-v010 rejects retired Stage-3 single_update/update_loop modes" in train
    ...
    assert 'G5-S4 bounded Stage 3 requires train mode, 64 envs, 1 iteration, and 1 update' in stage3
    ...
    assert 'FRS-TRAIN-v022 uses actual Actor LR=3e-7->1e-6 and Critic LR=1e-5 with B8/M4' in stage3
    ...
    assert '" --frontres_segment_cache_dir ${CACHE_DIR} "' in stage3
    assert '" --frontres_segment_shard_cache_size ${SHARD_CACHE_SIZE} "' in stage3
    assert 'FRS-PPO-v010 rejects retired optimizer-writing Stage 3 mode' in stage3
