...
class OnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        self._frontres_critic_current_observation_dim = int(num_privileged_obs)
        ...
        if self.training_type == "frontres" and (
            ...
            print(
                ...
                f"frontres_obs={authority.frontres_visible_dim} critic_obs={num_privileged_obs} "
                f"gmt_obs={authority.gmt_suffix_dim}",
                flush=True,
            )
    ...
    def finalize_frontres_local_sentinel_checkpoint(self, result: object) -> str:
        """Save the exact checkpoint-v16 adjacent to one completed local sentinel."""
            ...
            with torch.inference_mode(): # 关闭计算图的梯度追踪, 只进行推理
                ...
                for _rollout_step in range(self.num_steps_per_env):
                    ...
                    if (
                        ...
                        raise RuntimeError(
                            "FRS-TRAIN-v021 forbids frontres_hsl_rollout_label_enabled on every active Stage-3 route"
                        )
