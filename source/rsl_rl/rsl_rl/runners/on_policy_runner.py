...
class OnPolicyRunner:
    ...
    def evaluate_frontres_dr_sweep(
        ...
        *,
        ...
        if self.training_type != "frontres" or not isinstance(self.alg.policy, FrontRESActorCritic):
        ...
        use_quartet = bool(self.cfg.get("frontres_candidate_rollout_enabled", False))
        n_pair = self.env.num_envs // (4 if use_quartet else 3)
        n_train = n_pair
        ...
        def _policy_obs_from_extras(_obs: torch.Tensor, _extras: dict) -> tuple[torch.Tensor, torch.Tensor | None]:
            ...
            if self.policy_obs_type is not None and self.policy_obs_type in _obs_dict:
                _obs = _obs_dict[self.policy_obs_type]
            _obs = self._apply_obs_normalizer(_obs.to(self.device))
            _ref_vel_obs = None
            if self.ref_vel_estimator_obs_type is not None and self.ref_vel_estimator_obs_type in _obs_dict:
                _ref_vel_obs = _obs_dict[self.ref_vel_estimator_obs_type].to(self.device)
        ...
        def _frontres_policy_input(_obs: torch.Tensor, _ref_vel_obs: torch.Tensor | None) -> torch.Tensor:
            if (
                ...
                _estimated_ref_vel = self.alg.ref_vel_estimator(_estimator_input)
        ...
        def _allowed_bases() -> tuple[str, ...]:
            ...
            channels = str(self.cfg.get("frontres_perturbation_channels", "") or "").lower()
        ...
        def _apply_fixed_scale(scale: float) -> None:
            ...
            self._frontres_curriculum_active_modes = tuple(allowed_bases)
            self._frontres_curriculum_env_mode_groups = [tuple(allowed_bases)] * n_train
            self._frontres_curriculum_mix_label = "fixed_eval"
            self._frontres_dr_scale_mean_last = float(scale)
            self._frontres_dr_mix_ratio_easy = 0.0
            self._frontres_dr_mix_ratio_frontier = 1.0
            self._frontres_dr_mix_ratio_hard = 0.0

        max_episode_length = float(getattr(self.env, "max_episode_length", 0) or 0)
        rollout_steps = max(1, int(num_iterations_per_scale)) * int(self.num_steps_per_env)
        results: list[dict] = []
        was_training = self.alg.policy.training
        self.eval_mode()
        try:
            with torch.inference_mode():
                for scale in [float(value) for value in dr_scales]:
                    ...
                    for _ in range(rollout_steps):
                        task_corr = self.alg.policy.get_task_correction_inference(obs)
                        ...
                        if (
                            ...
                            alpha = self.alg.policy.get_state_router_alpha(obs[:n_train]).view(-1).detach()
                            self._frontres_state_alpha_prob_next = alpha
                            self._frontres_state_alpha_pred_last = float(alpha.mean().item())
                        else:
                            self._frontres_state_alpha_prob_next = torch.zeros(n_train, device=self.device)
                            self._frontres_state_alpha_pred_last = 0.0

                        self._frontres_stable_route_next_mask = torch.zeros(
                            n_train, device=self.device, dtype=torch.bool
                        )
                        ...
                        self.alg.policy._cached_observations = obs_corr
                        env_actions = self.alg.policy.get_env_action(obs_corr, task_corr)
                        obs, _, dones, infos = self.env.step(env_actions.to(self.env.device))
                        dones = dones.to(self.device).view(-1)
                        cur_episode_length += 1.0
                        time_outs = infos.get("time_outs", torch.zeros_like(dones))
                        time_outs = time_outs.to(self.device).view(-1).bool()
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):  # noqa: C901
        ...
        for it in range(start_iter, tot_iter):
            ...
            _frontres_rho_regret_up_planar_sum: float = 0.0
            _frontres_rho_regret_up_rp_sum: float = 0.0
            _frontres_rho_regret_up_z_sum: float = 0.0
            _frontres_rho_regret_down_planar_sum: float = 0.0
            _frontres_rho_regret_down_rp_sum: float = 0.0
            _frontres_rho_regret_down_z_sum: float = 0.0
            ...
            _frontres_oracle_ub_candidate_win_sum: float = 0.0
            _frontres_oracle_ub_feasible_win_sum: float = 0.0
            _frontres_oracle_ub_noisy_win_sum: float = 0.0
                ...
                for _rollout_step in range(self.num_steps_per_env):
                    ...
                    if _is_frontres:
                        ...
                        if _use_clean:
                            ...
                            if bool(self.cfg.get("frontres_oracle_upper_bound_diag_enabled", True)):
                                _oracle_ub_scores = torch.stack(
                                    (_exec_perturbed, _exec_frontres, _exec_candidate, _exec_feasible),
                                    dim=0,
                                )
                                _oracle_ub_best, _oracle_ub_src = _oracle_ub_scores.max(dim=0)
                                _oracle_ub_gain = _oracle_ub_best - _exec_perturbed
                                _oracle_ub_margin = float(
                                    self.cfg.get("frontres_oracle_upper_bound_margin", 0.0)
                                )
                                _oracle_ub_pass = (_oracle_ub_gain > _oracle_ub_margin).float()
                                _oracle_ub_noisy_win = (_oracle_ub_src == 0).float()
                                _oracle_ub_projected_win = (_oracle_ub_src == 1).float()
                                _oracle_ub_candidate_win = (_oracle_ub_src == 2).float()
                                _oracle_ub_feasible_win = (_oracle_ub_src == 3).float()
                            else:
                                _oracle_ub_gain = torch.zeros_like(_repair_gain)
                                _oracle_ub_pass = torch.zeros_like(_repair_gain)
                                _oracle_ub_noisy_win = torch.ones_like(_repair_gain)
                                _oracle_ub_projected_win = torch.zeros_like(_repair_gain)
                                _oracle_ub_candidate_win = torch.zeros_like(_repair_gain)
                                _oracle_ub_feasible_win = torch.zeros_like(_repair_gain)
                            ...
                            _rho_regret_up_planar_mean = torch.tensor(0.0, device=self.device)
                            _rho_regret_up_rp_mean = torch.tensor(0.0, device=self.device)
                            _rho_regret_up_z_mean = torch.tensor(0.0, device=self.device)
                            _rho_regret_down_planar_mean = torch.tensor(0.0, device=self.device)
                            _rho_regret_down_rp_mean = torch.tensor(0.0, device=self.device)
                            _rho_regret_down_z_mean = torch.tensor(0.0, device=self.device)
                            ...
                            self._frontres_rho_regret_up_planar_last = 0.0
                            self._frontres_rho_regret_up_rp_last = 0.0
                            self._frontres_rho_regret_up_z_last = 0.0
                            self._frontres_rho_regret_down_planar_last = 0.0
                            self._frontres_rho_regret_down_rp_last = 0.0
                            self._frontres_rho_regret_down_z_last = 0.0
                            ...
                            if _pref_enabled:
                                ...
                                if _rho_space in ("tri_anchor", "tri-anchor", "tri"):
                                    ...
                                    if _grouped_targets_enabled:
                                        ...
                                        def _component_branch(_name: str, _start: int, _fallback: torch.Tensor) -> torch.Tensor:
                                            _component = _comp.get(_name, _fallback)
                                            return _component[_start:_start + _n_exec].to(self.device)

                                        def _component_gain(_name: str, _fallback: torch.Tensor) -> torch.Tensor:
                                            return (
                                                _component_branch(_name, _candidate_start, _fallback)
                                                - _component_branch(_name, _base_start, _fallback)
                                            )

                                        def _group_score(_start: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
                                            _xy = _component_branch("xy", _start, _comp["planar"])
                                            _yaw = _component_branch("yaw", _start, _comp["planar"])
                                            _rp = _component_branch("rp", _start, _comp["vertical"])
                                            _z = _component_branch("z", _start, _comp["vertical"])
                                            _planar = 0.5 * (_xy + _yaw)
                                            return _planar, _rp, _z

                                        if _regret_target_enabled:
                                            _candidate_planar, _candidate_rp, _candidate_z = _group_score(
                                                _candidate_start
                                            )
                                            _projected_planar, _projected_rp, _projected_z = _group_score(0)
                                            _base_planar, _base_rp, _base_z = _group_score(_base_start)

                                            def _regret_target(
                                                _candidate_score: torch.Tensor,
                                                _projected_score: torch.Tensor,
                                                _base_score: torch.Tensor,
                                                _rho_group: torch.Tensor,
                                            ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
                                                _under = torch.relu(_candidate_score - _projected_score - _pref_margin)
                                                _over = torch.relu(_base_score - _projected_score - _pref_margin)
                                                _score_range = (_candidate_score - _base_score).abs().clamp(min=1e-6)
                                            ...
                                            _rho_rp_current = 0.5 * (_rho_current[:, 3] + _rho_current[:, 4])
                                            _rho_z_current = _rho_current[:, 2]
                                            _target_planar, _regret_up_planar, _regret_down_planar = _regret_target(
                                                _candidate_planar,
                                                _projected_planar,
                                                _base_planar,
                                                _rho_planar_current,
                                            )
                                            _target_rp, _regret_up_rp, _regret_down_rp = _regret_target(
                                                _candidate_rp,
                                                _projected_rp,
                                                _base_rp,
                                                _rho_rp_current,
                                            )
                                            _target_z, _regret_up_z, _regret_down_z = _regret_target(
                                                _candidate_z,
                                                _projected_z,
                                                _base_z,
                                                _rho_z_current,
                                            ...
                                            _regret_up_planar = torch.relu(_planar_gain - _pref_margin)
                                            _regret_up_rp = torch.relu(_rp_gain - _pref_margin)
                                            _regret_up_z = torch.relu(_z_gain - _pref_margin)
                                        ...
                                        _rho_regret_up_planar_mean = _regret_up_planar.mean()
                                        _rho_regret_up_rp_mean = _regret_up_rp.mean()
                                        _rho_regret_up_z_mean = _regret_up_z.mean()
                                        _rho_regret_down_planar_mean = _regret_down_planar.mean()
                                        _rho_regret_down_rp_mean = _regret_down_rp.mean()
                                        _rho_regret_down_z_mean = _regret_down_z.mean()
                                    else:
                                ...
                                if bool(_active_pref.any().detach().item()):
                                    ...
                                    if _structured_joint_enabled:
                                        _near = max(float(_pref_margin), 1.0e-6)
                                        _pref_full_frac = (_active_target > _near).float().mean()
                                        _pref_noop_frac = (_active_target < -_near).float().mean()
                                        _pref_keep_frac = (_active_target.abs() <= _near).float().mean()
                                    else:
                                ...
                                self._frontres_rho_regret_up_planar_last = float(
                                    _rho_regret_up_planar_mean.detach().item()
                                )
                                self._frontres_rho_regret_up_rp_last = float(
                                    _rho_regret_up_rp_mean.detach().item()
                                )
                                self._frontres_rho_regret_up_z_last = float(
                                    _rho_regret_up_z_mean.detach().item()
                                )
                                self._frontres_rho_regret_down_planar_last = float(
                                    _rho_regret_down_planar_mean.detach().item()
                                )
                                self._frontres_rho_regret_down_rp_last = float(
                                    _rho_regret_down_rp_mean.detach().item()
                                )
                                self._frontres_rho_regret_down_z_last = float(
                                    _rho_regret_down_z_mean.detach().item()
                                )
                        ...
                        if _r_z is not None:
                            ...
                            _frontres_rho_regret_up_planar_sum += float(
                                getattr(self, "_frontres_rho_regret_up_planar_last", 0.0)
                            )
                            _frontres_rho_regret_up_rp_sum += float(
                                getattr(self, "_frontres_rho_regret_up_rp_last", 0.0)
                            )
                            _frontres_rho_regret_up_z_sum += float(
                                getattr(self, "_frontres_rho_regret_up_z_last", 0.0)
                            )
                            _frontres_rho_regret_down_planar_sum += float(
                                getattr(self, "_frontres_rho_regret_down_planar_last", 0.0)
                            )
                            _frontres_rho_regret_down_rp_sum += float(
                                getattr(self, "_frontres_rho_regret_down_rp_last", 0.0)
                            )
                            _frontres_rho_regret_down_z_sum += float(
                                getattr(self, "_frontres_rho_regret_down_z_last", 0.0)
                            )
                            ...
                            _frontres_structured_joint_adv_sum += float(
                                getattr(self, "_frontres_structured_joint_adv_last", 0.0)
                            )
                            _frontres_structured_joint_weight_sum += float(
                                getattr(self, "_frontres_structured_joint_weight_last", 0.0)
                            )
                            ...
                            _frontres_oracle_ub_projected_win_sum += _oracle_ub_projected_win.mean().item()
                            _frontres_oracle_ub_candidate_win_sum += _oracle_ub_candidate_win.mean().item()
                            _frontres_oracle_ub_feasible_win_sum += _oracle_ub_feasible_win.mean().item()
                            _frontres_oracle_ub_noisy_win_sum += _oracle_ub_noisy_win.mean().item()
            ...
            frontres_oracle_ub_gain_mean = (
                _frontres_oracle_ub_gain_sum / _frontres_reward_diag_steps
                if _is_frontres and _frontres_reward_diag_steps > 0 else None
            )
            frontres_oracle_ub_pass_mean = (
                _frontres_oracle_ub_pass_sum / _frontres_reward_diag_steps
                if _is_frontres and _frontres_reward_diag_steps > 0 else None
            )
            frontres_oracle_ub_projected_win_mean = (
                _frontres_oracle_ub_projected_win_sum / _frontres_reward_diag_steps
                if _is_frontres and _frontres_reward_diag_steps > 0 else None
            )
            frontres_oracle_ub_candidate_win_mean = (
                _frontres_oracle_ub_candidate_win_sum / _frontres_reward_diag_steps
                if _is_frontres and _frontres_reward_diag_steps > 0 else None
            )
            frontres_oracle_ub_feasible_win_mean = (
                _frontres_oracle_ub_feasible_win_sum / _frontres_reward_diag_steps
                if _is_frontres and _frontres_reward_diag_steps > 0 else None
            )
            frontres_oracle_ub_noisy_win_mean = (
                _frontres_oracle_ub_noisy_win_sum / _frontres_reward_diag_steps
                if _is_frontres and _frontres_reward_diag_steps > 0 else None
            )
            ...
            frontres_rho_regret_up_planar_mean = (
                _frontres_rho_regret_up_planar_sum / _frontres_reward_diag_steps
                if _is_frontres and _frontres_reward_diag_steps > 0 else None
            )
            frontres_rho_regret_up_rp_mean = (
                _frontres_rho_regret_up_rp_sum / _frontres_reward_diag_steps
                if _is_frontres and _frontres_reward_diag_steps > 0 else None
            )
            frontres_rho_regret_up_z_mean = (
                _frontres_rho_regret_up_z_sum / _frontres_reward_diag_steps
                if _is_frontres and _frontres_reward_diag_steps > 0 else None
            )
            frontres_rho_regret_down_planar_mean = (
                _frontres_rho_regret_down_planar_sum / _frontres_reward_diag_steps
                if _is_frontres and _frontres_reward_diag_steps > 0 else None
            )
            frontres_rho_regret_down_rp_mean = (
                _frontres_rho_regret_down_rp_sum / _frontres_reward_diag_steps
                if _is_frontres and _frontres_reward_diag_steps > 0 else None
            )
            frontres_rho_regret_down_z_mean = (
                _frontres_rho_regret_down_z_sum / _frontres_reward_diag_steps
                if _is_frontres and _frontres_reward_diag_steps > 0 else None
            )
            ...
            frontres_structured_joint_adv_mean = (
                _frontres_structured_joint_adv_sum / _frontres_reward_diag_steps
                if _is_frontres and _frontres_reward_diag_steps > 0 else None
            )
            frontres_structured_joint_weight_mean = (
                _frontres_structured_joint_weight_sum / _frontres_reward_diag_steps
                if _is_frontres and _frontres_reward_diag_steps > 0 else None
            )
            frontres_structured_joint_rho_adv_mean = (
