    ...
    *,
    ...
    if is_task_space_mode and _uses_v015_future_intent_route(runner):
        if float(getattr(runner.alg, "lambda_supervised", 0.0)) > 0.0:
            raise RuntimeError(
            "FRS-TRAIN-v021 forbids a nonzero Stage-3 online HSL target writer on the v018 route"
            )
        return
...
def _capture_hsl_snapshot_before_step(
    ...
    *,
    ...
    if _uses_v015_future_intent_route(runner):
        raise RuntimeError(
            "FRS-TRAIN-v021 forbids legacy HSL rollout snapshots on the v018 Stage-3 route"
        )
