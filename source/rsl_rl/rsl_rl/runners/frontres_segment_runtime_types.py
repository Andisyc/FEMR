...
class FrontRESFormalTransactionRequest:
    ...
    def __post_init__(self) -> None:
        ...
        if isinstance(self.active_m, bool) or int(self.active_m) < 2:
            raise ValueError("FRS-TRAIN-v014 formal transaction active_m must be at least two")
        if self.plan.active_m != int(self.active_m) or self.plan.selected_segment_count != 2:
            raise ValueError("FRS-TRAIN-v014 formal transaction plan does not match exact two-Segment x M identity")
        ...
        if len(self.dr_stage_fingerprint) != 64:
            raise ValueError("FRS-TRAIN-v014 formal transaction requires a sealed DR-stage fingerprint")
        if not 0.0 <= float(self.dr_progress) <= 1.0 or not 0.0 < float(self.d_cap) <= 2.381:
            raise ValueError("FRS-TRAIN-v014 formal transaction has invalid DR progress or d_cap")
        if len(self.dr_class_by_segment) != 2 or len(self.dr_strength_by_segment) != 2:
            raise ValueError("FRS-TRAIN-v014 transaction requires one sealed DR class/strength per Segment")
        if any(name not in {"easy", "medium", "hard", "broken"} for name in self.dr_class_by_segment):
            raise ValueError("FRS-TRAIN-v014 transaction has an invalid DR class")
        if any(not math.isfinite(float(value)) or float(value) < 0.0 for value in self.dr_strength_by_segment):
            raise ValueError("FRS-TRAIN-v014 transaction has an invalid DR strength")
