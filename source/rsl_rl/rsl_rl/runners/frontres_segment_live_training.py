...
def finalize_frontres_local_sentinel_checkpoint(runner: Any, result: Any) -> str:
    """Persist and verify the checkpoint-v14 produced by one local sentinel update."""
    ...
    try:
        ...
        if not isinstance(identity, Mapping):
            raise RuntimeError("v017 local sentinel checkpoint has no checkpoint-v14 identity")
        required_identity = {
...
def _resolve_required_k_stage_handoff(runner: Any) -> FrontRESKStageIdentity | None:
    ...
    if observed_envs != previous_required_envs:
        raise RuntimeError(
            "FRS-TRAIN-v019 completed K-stage width drifted before process handoff: "
...
def run_frontres_segment_live_training_loop(
    ...
    *,
    ...
    if formal_v015 and absolute_start + num_learning_iterations > FRONTRES_V011_MAX_ABSOLUTE_ITERATION:
        raise RuntimeError(
            "FRS-TRAIN-v019 run would cross maximum_absolute_iteration=8000: "
        ...
        if formal_v015:
            ...
            if transition is not None:
                if runner.log_dir is None or runner.disable_logs:
                    raise RuntimeError(
                        "FRS-TRAIN-v019 K-stage process handoff requires an enabled committed checkpoint owner"
                    )
