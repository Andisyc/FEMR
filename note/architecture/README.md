...
- `concept/09_in_context_execution_calibration_design_inspector.data.json`: candidate Design Inspector for amortized FADA-style calibration. Eight parent cards share one episodic spine from hidden-condition sampling through Support encoding, fixed latent calibration, Query execution and Executed-Action supervision. It is a proposal review surface, not the active FrontRES Design Inspector or an implementation/runtime claim.
