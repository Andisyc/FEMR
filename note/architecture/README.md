...
- `runtime/04_frontres_design_inspector.data.json`: interactive Design Inspector: ten compact parent design points highlight where their fine-grained decisions change one shared Stage-3 Transaction spine. DP02/DP03/DP09 project the human-confirmed transition to low-DR joint Actor/Critic adaptation, four relative DR classes from the first warmup Transaction, phase-specific Critic-mean versus Repair-difference Replay priority, and current-DR-compatible replay pools. METHOD-v021 / TRAIN-v020 / PPO-v008 and checkpoint-v15 remain stale for these revised points until a later Contract migration.
- `runtime/04_frontres_design_register.md`: approved interaction and language contract for the Design Inspector.
- `testing/05_frontres_module_test_atlas.data.json`: Module Test Atlas with eighteen module cards plus a compact Formal Runtime Audit stage-reading card. All eighteen cards pass; TEST-05/15/16/18 carry fresh v016 evidence and the other fourteen retain their prior evidence. The module cards use human-readable `伪样本 | 正确结果 | 证明什么` cases.
...
- `concept/08_trajectory_conditioned_execution_alignment.data.json`: current candidate paper-style Concept Figure for offline within-Intent calibration. Its upper lane separates Support and Query into two independently pre-collected `Delta z = 0` data sources: Support sits beside the Context Encoder and produces a low-dimensional calibration latent; Query sits below the Tracker Encoder and supplies fixed Realized-Future/Executed-Action supervision. Only the Context Encoder receives the MSE gradient through the frozen Tracker. The Perturbation Condition is temporarily omitted while the Support–Query composition is reviewed. The dashed-separated lower lane retains the controlled real-world proof of concept from Action weakening to frozen corrected re-execution.
- `concept/09_in_context_execution_calibration_design_inspector.data.json`: candidate Design Inspector for amortized FADA-style calibration. Seven parent cards share one episodic spine from hidden-condition sampling through first-Rollout encoding, fixed Context calibration, Query execution and Executed-Action supervision. It is a proposal review surface, not the active FrontRES Design Inspector or an implementation/runtime claim.
...
- The eighteen module cards cover the formal runtime module families. The current v016 transition reports `18 passed / 0 partial / 0 blocked`; TEST-05, TEST-15, TEST-16 and TEST-18 have fresh confirmed evidence. Prior Phase A/B records characterize TRAIN-v015 only and cannot authorize the new route.
