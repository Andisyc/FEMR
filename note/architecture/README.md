...
- `runtime/04_frontres_design_inspector.data.json`: interactive Design Inspector: ten compact parent design points highlight where their fine-grained decisions change one shared Stage-3 Transaction spine. DP02 projects the active FRS-METHOD-v021 / TRAIN-v020 outer prioritized Scenario Replay: complete ScenarioKey identity, valid-only admission, utility-error priority, rank plus staleness selection, fixed global/replay/review timing, fresh current-policy M4 recollection and committed-only replay mutation.
