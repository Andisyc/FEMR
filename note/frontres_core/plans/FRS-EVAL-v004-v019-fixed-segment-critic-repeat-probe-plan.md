...
**Architecture:** Reuse the existing checkpoint installation, fixed-manifest materializer, Clean/Noisy/Repair collector, GAIN-v008 calculation, symlog utility, and atomic JSON writer. `repeat_count` is evaluation execution identity, not a manifest or method parameter. The evaluator caches the first repeat's complete policy inputs per Segment pair; the collector validates and reuses them after each later physical reset. The default remains one pass.
...
- State boundary: Actor/Critic/checkpoint and training owners are frozen; only global inference RNG may advance to sample independent actions. The first repeat seals the normalized 928D Actor and 449D Critic inputs. Later repeats may observe live-history drift for diagnostics but must consume exact clones of those sealed inputs.
...
- Sensitivity: scenario/hash drift, any used Actor/Critic-input drift, repeated action fingerprints, non-finite target, invalid repeat count, and K outside `{8,16}` must fail closed.
...
- Stop: any discrete identity drift, any used Actor/Critic-input drift, action non-diversity, training-state mutation, non-finite result, or failing existing evaluation contract.
...
- Repeat existing read-only collection in one checkpoint context; cache the first complete Actor/Critic input per Segment pair, assert exact used-input equality and diverse action fingerprints, and report live-history drift separately from Critic-value variation.
