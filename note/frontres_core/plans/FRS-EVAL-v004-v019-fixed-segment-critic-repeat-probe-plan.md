...
- State boundary: Actor/Critic/checkpoint and training owners are frozen; only global inference RNG may advance to sample independent actions.
...
- Sensitivity: scenario/hash/value drift, repeated action fingerprints, non-finite target, invalid repeat count, and K outside `{8,16}` must fail closed.
...
- Stop: any identity drift, action non-diversity, training-state mutation, non-finite result, or failing existing evaluation contract.
...
- Repeat existing read-only collection in one checkpoint context; assert fixed scenario/hash/value and diverse action fingerprints.
