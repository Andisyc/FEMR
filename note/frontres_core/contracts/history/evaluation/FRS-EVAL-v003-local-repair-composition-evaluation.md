...
Local evaluation reports the active v006 objective/constraint decomposition:
...
`E-FI-98` (2026-07-30) closes the deterministic paired-composition and runtime
boundary without changing the evaluation semantics:

- `frontres_runtime.py` owns one deployment runtime Gateway that contains all
  runner-private connectors, simulator/sensor IO, observation normalization,
  FEMR/GMT calls, route-start snapshot/restore and training-state fingerprint;
- one canonical route-start image is captured after initial reset and restored
  before sequential Baseline and Repair branches, including physical, command,
  perturber, CPU/CUDA RNG and sealed carrier identity;
- Baseline executes frozen GMT with an exact-zero 6D FEMR action and never
  invokes FEMR; Repair uses the same state/hash/carrier and invokes FEMR once
  per evaluated deployment frame;
- the immutable report contains both branch trajectories plus paired intent
  error and failure-count improvements, and rejects route-hash drift, Baseline
  FEMR use, nonzero Baseline action, mixed row shape or training-state mutation;
- the sequence evaluator consumes only the public Gateway and retains request,
  paired orchestration, pure metric reduction and atomic report ownership.

This is S1/S2 deterministic connectivity evidence. It does not establish
IsaacLab timing, real checkpoint quality, Demo quality or S4 deployment claims.
