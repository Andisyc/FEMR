...
- Stage 3 `frontres_segment_k=8` is the unknown-state/default horizon;
  `frontres_segment_max_horizon_k=64` unlocks the sampler-owned
  `8/16/32/64` curriculum in formal training.
- Mixed batches step the env to `max(horizon_k)`, but reward/done accumulation,
  discounted return construction, and motion-quality diagnostics use each
  row's own horizon. Quartet counterparts inherit the same K.
