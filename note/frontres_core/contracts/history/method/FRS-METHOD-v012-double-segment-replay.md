---
contract_id: FRS-METHOD-v012
status: superseded
effective_date: 2026-07-17
updated_date: 2026-07-19
supersedes: FRS-METHOD-v011
superseded_by: FRS-METHOD-v013
scope: FrontRES Stage 3 Double Segment Replay HRL method and PPO transaction semantics
---

# Double Segment Replay HRL Method Contract

> Historical contract. Superseded by FRS-METHOD-v013 on 2026-07-19; retain it
> only to interpret the v012 decision and its code-audit boundary.

## Concept Figure Mapping

| Design ID | Canonical human name | Figure block ID | Contract section |
| --- | --- | --- | --- |
| `FRS-DP-01` | Perturbation Data | `M-02` | `Perturbation Data` |
| `FRS-DP-02` | Segment Replay | `SR-01` | `Segment Replay Design` |
| `FRS-DP-03` | K-step Curriculum | `M-06` | `K-Step Curriculum` |
| `FRS-DP-04` | FrontRES 6D Repair | `M-04` | `Action Semantics` |
...
Perturbation Data constructs corrupted reference-motion cases from Clean
motion while preserving the identity needed for paired comparison. Every
sample must retain motion identity, start frame or phase, perturbation family,
strength, and the dynamic state needed by Segment Replay.

The perturbation family defines the corruption distribution only. It does not
define an action mask or narrow FrontRES repair dimensions. The current
specialist experiment may sample `local_rp`, while the policy still emits a
full six-dimensional repair. Composite families belong to later curriculum
stages after clear single-family repair signals are established.
...
The active method has two distinct replay layers:

```text
inner Segment trial replay
  -> keep one old policy snapshot fixed
  -> repeat multiple on-policy repair attempts for the same Segment visit
  -> estimate local Gain distribution and repair reliability

outer prioritized Segment replay
  -> use aggregated rollout evidence to choose global/replay/review Segments
  -> preserve dataset coverage while revisiting high-learning-value cases
```

The inner layer is not an optimizer loop. Policy parameters must remain fixed
while repeated attempts for the current collection transaction are generated.
One noisy attempt is insufficient evidence for a Segment-level policy update.

After collecting repeated attempts for multiple Segments under the same old
policy snapshot, PPO may update once from the eligible on-policy rows. Manual
search/counterfactual actions remain sampler evidence and cannot receive PPO
credit without matching old-policy log probabilities.

```text
freeze old policy
-> select multiple Segments
-> collect repeated on-policy attempts per Segment
-> aggregate Segment-local evidence
-> construct one cross-Segment PPO batch
-> optimizer.step once
```
...
The active path forbids one policy attempt for a Segment triggering an
immediate PPO update before its fixed-policy trial budget is collected.
Repeated visits under already changed policies are not a substitute for
fixed-policy multi-trial evidence.
...
Here `policy action` means the collected set of eligible on-policy attempts
from the fixed-policy transaction, not a single first trial chosen only because
it is labeled `policy`.
...
- A Segment contributes one noisy policy attempt and triggers an immediate PPO
  update before its fixed-policy trial budget is collected.
- Repeated Segment visits under changed policies are treated as fixed-policy
  multi-trial evidence.
...
- old-policy transaction identity, per-Segment on-policy attempt count, and
  optimizer-step count per completed collection transaction;
...
- Paired reward semantics: `../../active/reward/FRS-GAIN-v002-style-physics-repair.md`.
- PPO advantage semantics: `../optimization/FRS-PPO-v001-sign-preserving-advantage-scaling.md`.
- Periodic and sequence evaluation: `../../active/evaluation/FRS-EVAL-v002-segment-evaluation.md`.
