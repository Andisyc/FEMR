...
It is applied per attempt before M4 reduction. Raw Gain remains unchanged and
visible. The existing non-amplifying Critic loss scale remains a numerical
gradient conditioner in utility space. This remains PPO with `V(s)`; the 6D
action never enters the Critic and no Q-learning authority is introduced.
...
FRS-GAIN-v008 raw G_total, fixed utility return, and utility advantage
...
advantages produced by FRS-GAIN-v008.
