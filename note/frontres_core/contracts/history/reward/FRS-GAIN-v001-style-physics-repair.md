...
effective horizon K. Per-step components are accumulated only through that
row's K, then converted into paired gains. Done/survival semantics must be
...
prints every raw component and both gains separately; `gain_total` alone is
never sufficient evidence of method quality.
