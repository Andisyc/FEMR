---
contract_id: FRS-TRAIN-v013
status: superseded
effective_date: 2026-08-03
updated_date: 2026-08-03
supersedes: FRS-TRAIN-v012
scope: HSL-to-HRL Recovery-Aware scalar training, coordinated K x exact-M schedule, per-K lower-to-higher DR curriculum, Critic recalibration, actor ramp, grouped scalar PPO, and strict checkpoint-v8 persistence
---

# Nested K-DR Recovery-Aware Training Curriculum
...
FRS-TRAIN-v013 preserves the proven K8/M2 -> K16/M3 -> K32/M4 transaction
...
- FRS-GAIN-v007, FRS-PPO-v005, HSL-v1, beta, full-6D cost, single-`local_rp`,
...
resolve and seal TRAIN-v013 curriculum identity
enter K8/M2 critic_only with actor_loss_weight=0
```

HSL initialization and strict Stage-3 resume are mutually exclusive. Old
Stage-3 Critic, optimizer, transaction or curriculum state cannot initialize a
fresh v013 campaign.
...
## Checkpoint-v8 Identity

```text
checkpoint_schema = frontres-v015-checkpoint-v8
method_contract_id = FRS-METHOD-v017
gain_contract_id = FRS-GAIN-v007
optimization_contract_id = FRS-PPO-v005
training_contract_id = FRS-TRAIN-v013
...
Checkpoint-v7, v012 `g_8/g_16/g_32`, unversioned payloads, mismatched DR specs,
...
- checkpoint-v8 and all active contract identities;
...
Gain/PPO feedback, committed-only progress, checkpoint-v8 roundtrip and v7/
`g_K` pre-mutation rejection. Formal and live evidence remain separate gates.

Stop if starting/advance parameters are hidden defaults; an episode-length or
