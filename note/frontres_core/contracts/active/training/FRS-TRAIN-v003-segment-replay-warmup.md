...
| --- | --- | --- | --- |
| `FRS-DP-08` | HSL Warmup | `M-03` | `HSL Warmup` |
| `FRS-DP-09` | Actor & Critic Warmup | `M-05` | `Actor And Critic Warmup` |

## HSL Warmup

Stage 2 HSL provides the supervised initialization for the same full-6D actor
that Stage 3 later optimizes. It learns a proposal-only Delta SE(3) repair from
the active 870D ZMP/balance observation. It does not append confidence, rho,
acceptance, or family-specific action dimensions.

The Stage 2 checkpoint must preserve the actor architecture and observation
normalizer state required by Stage 3. Stage 3 must verify checkpoint identity,
full-6D head compatibility, and normalizer restoration before PPO updates are
allowed. HSL quality is initialization evidence; it does not replace paired
Noisy/Repaired executable evidence.

## Actor And Critic Warmup

Stage 3 must not expose the HSL-initialized actor immediately to unrestricted
joint PPO updates. The warmup schedule protects the pretrained repair mapping
while the value function learns the Segment Replay return distribution.

The required phase semantics are:

```text
HSL initialization
-> critic warmup with the actor held fixed against PPO updates
-> actor warmup with gradually introduced PPO actor updates
-> joint Actor/Critic Segment PPO training
```

Critic warmup trains value prediction on rollout returns without moving the
policy distribution. Actor warmup then introduces the direct full-6D PPO actor
objective progressively while the critic continues learning. Exact iteration
counts and ramps are configuration, but the phase identity, actor gradient
boundary, and effective actor weight must reach formal training and
diagnostics.

This schedule protects optimization state; it does not revive retired
acceptance, rho, authority, active-dimension masks, or actor gates. Throughout
all phases, perturbation family remains separate from the six-dimensional
policy output.
...
```text
Stage 3 config
-> load and verify the HSL full-6D actor and observation normalizer
...
- active training phase and phase iteration;
- effective critic and actor update weights;
- actor parameter delta during critic warmup;
- HSL checkpoint and observation-normalizer load identity;
...
Warmup acceptance additionally requires:

1. S1 proof that critic warmup changes critic parameters while actor parameters
   remain unchanged.
2. S1 proof that actor warmup introduces actor updates monotonically according
   to the configured schedule without changing the full-6D action identity.
3. S2 connectivity proof that formal Stage 3 selects and propagates each phase.
4. S3 checkpoint/resume proof that phase counters and optimizer state resume
   consistently.
5. S4 live evidence for phase, actor weight, critic loss, actor parameter delta,
   and post-update trust-region diagnostics before full training.
