...
| PHASE-B-01 | live S4 | official train entry emits active contract/HSL-v2/K8-M2/offset `(1,2)` identity with no legacy route | planned; probe not inserted | AUDIT-B01 / Runtime Audit Atlas R01 |
| PHASE-B-02 | live S4 | sampler emits two sealed Segments x exact M=2 with four immutable policy rows and no reset resampling | planned; probe not inserted | AUDIT-B02 / R02 |
| PHASE-B-03 | live S4 | real reset/command/observation route proves B=8 roles and `870+58 -> 928 -> 158/770` with Noisy provenance | planned; probe not inserted | AUDIT-B03 / R03 |
| PHASE-B-04 | live S4 | each attempt has one finite `[6]` action, then FEMR freezes while eval/no-grad GMT executes K8 | planned; probe not inserted | AUDIT-B04 / R04 |
| PHASE-B-05 | live S4 | Clean=2, Noisy=2 and Repair=4 feed complete v007 `G_I/G_P/P_N/P_R/lambda/cost/G_total` evidence | planned; probe not inserted | AUDIT-B05 / R05 |
| PHASE-B-06 | live S4 | storage writes four policy rows, not K-expanded rows; return equals G_total and identity remains sealed | planned; probe not inserted | AUDIT-B06 / R06 |
| PHASE-B-07 | live S4 | grouped loss gives two Segments equal influence and exactly one optimizer update; K8 critic-only freezes Actor/std | planned; probe not inserted | AUDIT-B07 / R07 |
| PHASE-B-08 | live S4/S3 | one committed receipt advances iteration/curriculum and saves checkpoint-v9 with GMT/layout identity | planned; probe not inserted | AUDIT-B08 / R08 |
| PHASE-B-ARTIFACT | authority | a real `frontres-v017-hsl-proposal-v2` absolute path is verified before command release | blocked for human input | old HSL-v1 path cannot be reused by assumption |
