...
| `frontres_hsl_v007_s1_contract.py` | S1 | v007 q29 Stage-1 input/provenance, current anti-DR target, legacy label and direct-writer rejection, zero/rejected loss configuration, and legacy HSL checkpoint rejection. |
...
| `frontres_v015_deployment_live_cli_s4s0_contract.py` | S2 complete, `E-FI-31` | Absolute FEMR/GMT/NPZ/report identities, persistent protocol, CUDA-visible mapping, registered Unified task, eval-only agent config, no sampler request, formal checkpoint order, S2B-only dispatch, and zero optimizer-step delta. |
| Planned bounded deployment-composition sentinel | S4 missing | One identified pre-materialized deployment stream through the S4-S0 CLI and S2B owner with physical per-frame telemetry and zero feedback; requires a separately authorized live run. |
