...
| `frontres_hsl_v007_s1_contract.py` | S1/S2/S3 | v007 proposal carrier and formal `(1,2)` `928/158/770` actor-only HSL; strict HSL v1 persistence and pre-mutation rejection; bounded-smoke config/telemetry schema; an independent pre-warmup shadow reproduces exact normalized 158D input and bounded 6D proposal after reload while critic/optimizer/Stage-3 HSL remain isolated. |
