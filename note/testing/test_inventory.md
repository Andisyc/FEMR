...
| `frontres_v017_policy_quality_eval_contract.py` | S2 | Fixed K16/M3 manifest -> four read-only two-Segment transactions -> existing v017 collector/Gain report -> atomic JSON; evaluation-only aggregate lifecycle preserves the committed receipt, rejects stale/training context and cleans exceptions; missing Gain beta and training-state mutation fail closed. |
